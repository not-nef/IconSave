...
void IconSelectPopup::scrollWheel(float _dy, float _dx) {
    auto lr = this->m_scrollingLayer->m_scrollLayer;
    ...
    if (dest > this->m_scrollingLayer->m_layerHeight)
        lr->setPositionY(
            this->m_scrollingLayer->m_layerHeight
        );
}
...
void IconSelectPopup::onClose(cocos2d::CCObject* pSender) {
    ...
    Mod::get()->patch(reinterpret_cast<void*>(base::get() + 0x2db6f), { 0x6a, 0x28, 0x6a, 0x14, 0x6a, 0x0 });
...
void IconSelectPopup::showKitRemovedMessage(float _y) {
    ...
    bgSprite->setPosition(
        this->m_scrollingLayer->getScaledContentSize().width / 2,
    ...
    this->m_scrollingLayer->m_scrollLayer->addChild(bgSprite);
}
...
void IconSelectPopup::showPage(unsigned int _page, const char* _filter) {
    ...
    this->m_scrollingLayer->m_scrollLayer->removeAllChildrenWithCleanup(true);
    this->m_scrollingLayer->m_scrollLayer->setPositionY(0.0f);
    ...
    for (auto ix = 0u; ix < count; ix++) {
        ...
        this->m_scrollingLayer->m_scrollLayer->addChild(kitWidget);
...
void IconSelectPopup::setup() {
    ...
    Mod::get()->patch(reinterpret_cast<void*>(base::get() + 0x2db6f), { 0x6a, 0x1a, 0x6a, 0x29, 0x6a, 0x4c });
    ...
    this->m_mainLayer->addChild(this->m_pPageLabel);
    ...
    if (kits->count()) {
        ...
        this->m_pBtnLeft = CCMenuItemSpriteExtra::create(
        ...
        this->m_buttonMenu->addChild(this->m_pBtnRight);
        if (this->m_nCurrentPage == maxPage)
            this->m_pBtnRight->setVisible(false);
        ...
        this->m_mainLayer->addChild(noKitsLabel, 105);
    }

    this->m_mainLayer->addChild(this->m_scrollingLayer);
    ...
    this->m_buttonMenu->addChild(searchBtn, 100);
    ...
    this->m_buttonMenu->addChild(cancelBtn, 100);
